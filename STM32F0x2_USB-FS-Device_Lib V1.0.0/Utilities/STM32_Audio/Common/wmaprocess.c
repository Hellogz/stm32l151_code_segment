...
#if defined(__WMA_DECODER__) /* Don't build if not configured for use in audio_app_conf.h */
                                ...
                                fnSetPositionCallback_TypeDef* pSetPosCallback)
{
  ...
  aiPersist = (uint32_t *)WMA_DEC_MALLOC(SPIRIT_WMADEC_PERSIST_SIZE_IN_BYTES);
  if (aiPersist == NULL)
  {
    return 1;
  }
  ...
  aiScratch = (uint32_t *)WMA_DEC_MALLOC(SPIRIT_WMADEC_SCRATCH_SIZE_IN_BYTES);
  if ( aiScratch == NULL )
  {
    WMA_DEC_FREE(aiPersist);
    aiPersist = NULL;
    return 2;
  }
  
  /* Allocate temporary buffer for ASF decoding */
  acBufIn = (uint8_t*) WMA_DEC_MALLOC(4 * 1024 * sizeof(uint8_t));
  if (acBufIn == NULL)
  {
    WMA_DEC_FREE(aiScratch);
    aiScratch = NULL;
    WMA_DEC_FREE(aiPersist);
    aiPersist = NULL;
    return 3;
...
/**
  * @brief  Get the length of the current wma stream.
  * @param  None.
...
/**
  ...
  * @param  pFile: pointer to the data file.
  * @retval None.
  */
                                             ...
                                             void *user_prm)
{
  ...
  NumBytes = pWMAReadCallback (acBufIn, uiNumBytes, user_prm);
  *ppubBuf = (unsigned char *)acBufIn;
