...
#ifdef LPM_ENABLED      
      if ((remotewakeupon ==0)&&(L1resume == 0))
      {
        Resume_Init();
        ResumeS.eState = RESUME_OFF;
      }
      else if (remotewakeupon ==1)/* RESUME detected during the L2 RemoteWAkeup signalling => keep RemoteWakeup handling*/
      {
...
#ifdef LPM_ENABLED
  case L1_RESUME_INTERNAL:
    Resume_Init();
    /* apply L1 remote wakeup */
    wCNTR = _GetCNTR();
    wCNTR |= CNTR_L1RESUME;
    _SetCNTR(wCNTR);  
    L1resume =1;
